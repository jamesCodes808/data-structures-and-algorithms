...
const screenForNames = (arr) => {
  let filteredArr = [];
  arr.forEach(item => {
    if ((/^(Mr\.|Mrs\.|Ms\.|Dr\.)\s?[a-zA-Z]+$/).test(item)) {
...
const toTitleCase = (arr) => {
  return arr.map(item => {
    let capitalizedItem = item.charAt(0).toUpperCase() + item.substring(1, item.length)
...
let biggerThanLuke = (arr) => {
  ...
  const luke = arr.filter(character => character.name === 'Luke Skywalker');

  let arrOfBiggerMass = [];

  arr.forEach(character => {
    if (parseInt(character.mass) > parseInt(luke[0].mass)) {

      arrOfBiggerMass.push(character.name)
    }
  })


  let joinedArr = arrOfBiggerMass.join(' - ')
  return joinedArr;
