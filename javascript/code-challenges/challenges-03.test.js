...
const addTwo = (arr) => {
  let arrAddTwo = arr.map(num => num + 2)
  return arrAddTwo;
};
...
const typeNum = (arr) => {
  let arrOnlyNum = arr.filter(item => {
...
const containsAnd = (arr) => {
  let arrContainsAnd = arr.filter(item => {
...
const oddValues = (arr) => {
  let arrOddValues = arr.filter(num => num % 2 === 1);
  return arrOddValues;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let arrNotInFirstArray = arr.filter((item => !forbiddenValues.includes(item)));
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let arrBaseStatGreaterThan = new Array();

  arr.filter(item => {
    if (item.baseStat > minBaseStat) {
      arrBaseStatGreaterThan.push(item);
    }
  });

  return arrBaseStatGreaterThan;
};
...
const getStatName = (arr, minBaseStat) => {
  let arrStatName = new Array();

  arr.filter(item => {
    if (item.baseStat > minBaseStat) {
      arrStatName.push(item.stat.name);
    }
  });
...
const getCharactersWithoutChildren = (arr) => {
  let arrWithoutChildren = arr.filter(obj => !obj.hasOwnProperty('children'))
  return arrWithoutChildren;
};
...
const evenOddNumericValues = (arr) => {
  let arrNumsOnly = arr.filter(item => typeof item === 'number');
  let arrEvenOdd = new Array();

  arrEvenOdd = arrNumsOnly.map(item => {
    if (item % 2 == 0) {
      return 'even'
    } else if (item % 2 == 1) {
      return 'odd'
