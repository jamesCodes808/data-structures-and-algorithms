...
const longestString = (arr) => {
  ...
  // console.log(arr.indexOf('Tangerine'))

  // let indexOfLongestString = 0;

  // for (let i = 1; i < arr.length; i++) {
  //   if (arr[i].length > arr[indexOfLongestString].length) {
  //     console.log('first if', arr[i])
  ...
  //     indexOfLongestString = indexOfLongestString
  //   }
  // }
  let indexOfLongestString = 0;
  ...
  arr.reduce((acc, curr, idx) => {
    if (curr.length > acc) {
      console.log('if curr > acc', idx)
      console.log(acc)
      indexOfLongestString = idx
      acc = curr.length;
    }
    return acc;
  }, 0)

  console.log(indexOfLongestString)
...
const findHappiness = (arr) => {
  return arr.filter(item => { if (item.includes(':)')) { return item } })
};
...
const onlyOddChars = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    if (i % 2 !== 0) {
      arrOfOdds.push(str[i])
    }
  }
  let joined = arrOfOdds.join('');
  return joined
};
...
const allHappy = (arr) => {
  let originalArr = arr;
  let checkedLength = [];
  checkedLength = arr.filter(item => { if (item.includes(':)')) { return item } })



  if (checkedLength.length === originalArr.length) {
    return true;
  } else {
    return false
  }
