...
const toLastNames = people => {
  let arrFullNames = people.map(person => person.firstName + " " + person.lastName);
  return arrFullNames;
};
...
const countNumberOfElements = (arr) => {
  return arr.reduce((acc, item, idx) => acc + idx - 1, 0)
};
...
const reversedString = (str) => {
  ...
  return arrOfChar.reduce((acc, char) => char + acc, '')
};
...
const calculateAverage = (arr) => {
  let accumulatedNums = arr.reduce((prevNum, currNum) => {

    return {
      ...prevNum, ["count"]: arr.length,
      ...prevNum, ["sum"]: currNum + (prevNum.hasOwnProperty('sum') ? prevNum.sum : 0)
    };
  }, {})

  return accumulatedNums.sum / accumulatedNums.count
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((prevNum, currNum) => isPrime(currNum) ? prevNum + 1 : prevNum + 0, 0)
};
...
const extractStats = (snorlaxData) => {
  let extractedStats = snorlaxData.stats.reduce((prev, curr) => {
    return { ...prev, [curr.stat.name]: curr.baseStat }
  }, {})
...
const extractChildren = (arr) => {
  let charactersWithAInName = arr.filter((character) => character.name.includes('a'));


  let childrenOfCharactersWithAInName = charactersWithAInName.reduce((prev, curr) => {
  ...
  return childrenOfCharactersWithAInName;
};
